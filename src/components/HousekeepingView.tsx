...
export const HousekeepingView: React.FC = () => {
  const { housekeeping, rooms, updateHousekeepingStatus, toggleHousekeepingChecklist, setHousekeeping } = useApp();

  const [selectedDate, setSelectedDate] = useState<string>(() => housekeeping[0]?.date || new Date().toISOString().split('T')[0]);

  const todayTasks = housekeeping.filter(h => h.date === selectedDate);
  ...
  const handleGenerateTasksForDate = () => {
    const newTasks: HousekeepingTask[] = rooms.map((r, i) => ({
      id: `hk-${Date.now()}-${i}`,
      roomId: r.id,
      roomName: r.name,
      date: selectedDate,
      ...
      status: 'to_do',
      assignedTo: i % 2 === 0 ? 'Marie' : 'Sophie',
      checklist: [
        { id: `c1-${Date.now()}-${i}`, label: 'Aération & Dépoussiérage', completed: false },
        { id: `c2-${Date.now()}-${i}`, label: 'Changement des draps & lits au carré', completed: false },
        { id: `c3-${Date.now()}-${i}`, label: 'Nettoyage & désinfection salle de bain', completed: false },
        { id: `c4-${Date.now()}-${i}`, label: 'Réapprovisionnement linge & produits d\'accueil', completed: false },
        { id: `c5-${Date.now()}-${i}`, label: 'Aspiration & lavage des sols', completed: false }
      ],
      notes: 'Contrôle standard avant arrivée'
    }));

    setHousekeeping([...housekeeping, ...newTasks]);
  };
    ...
    <div className="space-y-4 pb-8">
      ...
      <div className="bg-white p-2.5 rounded-lg border border-stone-200 shadow-2xs flex items-center justify-between text-xs">
        <div className="flex items-center space-x-2">
          ...
          <span className="font-bold text-stone-700">Date d'intervention :</span>
          <input
            type="date"
            value={selectedDate}
        ...
        {todayTasks.length === 0 && (
          <button
            onClick={handleGenerateTasksForDate}
            className="flex items-center space-x-1.5 bg-[#4A6741] hover:bg-[#3d5636] text-white font-medium px-3 py-1 rounded text-xs transition cursor-pointer"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>Générer planning pour le {selectedDate}</span>
          </button>
        )}
      </div>
      ...
      {todayTasks.length > 0 ? (
        ...
        <div className="bg-white p-8 text-center rounded-lg border border-stone-200 space-y-3">
          <BedDouble className="w-8 h-8 text-stone-300 mx-auto" />
          <h3 className="font-bold text-sm text-stone-800">Aucune tâche de ménage programmée pour le {selectedDate}</h3>
          <p className="text-xs text-stone-500 max-w-sm mx-auto">
            Cliquez sur "Générer planning" pour créer automatiquement les checklists de ménage de toutes vos chambres pour cette date.
          </p>
          <button
            onClick={handleGenerateTasksForDate}
            className="inline-flex items-center space-x-1.5 bg-[#4A6741] hover:bg-[#3d5636] text-white font-medium px-4 py-2 rounded text-xs transition cursor-pointer"
          >
            <Sparkles className="w-3.5 h-3.5" />
            <span>Générer les tâches de ménage</span>
          </button>
        </div>
      )}
    </div>
